...
public class Related_Adapter extends RecyclerView.Adapter<Related_Adapter.MyViewHolder> {

    Context context;
    ArrayList<DetailsDataModel> alist;
    ...
    public Related_Adapter(Context context, ArrayList<DetailsDataModel> alist) {
        this.context = context;
        this.alist = alist;
    }

    @Override
    public Related_Adapter.MyViewHolder onCreateViewHolder(ViewGroup viewGroup, int i) {
        View itemView = LayoutInflater.from(viewGroup.getContext()).inflate(R.layout.relatedproducts, viewGroup, false);

        return new Related_Adapter.MyViewHolder(itemView);
    }

    @Override
    public void onBindViewHolder(Related_Adapter.MyViewHolder myViewHolder, final int i) {

        myViewHolder.tv_name.setText(alist.get(i).getProd_name());
        Glide.with(context).load(alist.get(i).getGallery1()).placeholder(R.drawable.no_image).into(myViewHolder.iv_image);
        myViewHolder.tv_view.setText(alist.get(i).getSize());
